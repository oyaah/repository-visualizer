...
export function GraphCanvas({ graph, selectedNodeId, onSelectNode }: Props) {
  ...
  const packageFlow = buildPackageFlow(graph.packages, graph.package_edges);

  function openPackage(name: string) {
    setViewMode('files');
    setGraphMode('all');
    setFolder('all');
    setQuery(name === '(root)' ? '' : name);
  }
  ...
  return (
    <section className="graph-panel" aria-label="Repository dependency graph">
      <div className="graph-toolbar">
        ...
        {viewMode === 'packages' ? (
          <>
            <strong>{graph.packages.length} packages</strong>
            <span>{graph.package_edges.length} cross-package links</span>
  ...
  return (
    <div className={`repo-node ${tone} ${selected ? 'selected' : ''}`}>
      ...
      <div className="repo-node-metrics">
        ...
        <span className={`risk-pill ${tone}`}>risk {Math.round(node.risk)}</span>
      </div>
      {node.git ? (
  ...
  return (
    <div className={`package-node ${tone}`} title={pkg.name}>
      ...
      <div className="package-node-top">
        <strong>{leaf}</strong>
        <span className={`risk-pill ${tone}`}>risk {Math.round(pkg.risk)}</span>
      </div>
...
function matchesPreset(node: GraphNode, preset: GraphPreset): boolean {
  ...
  if (preset === 'issues') {
    return node.unresolved_imports.length > 0 || node.metrics.complexity >= 8 || node.metrics.loc >= 80 || node.risk >= 60;
  }
  return true;
}
